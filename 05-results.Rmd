...
```{r}
finance_point <- financeTable %>%
  mutate(Employees = ifelse(is.na(Employees), 0, Employees)) %>%
  mutate_all(function(x) as.numeric(as.character(gsub(",","",x)))) 
colnames(finance_point) <- abbreviate(names(finance_point), minlength=2) 
finance_point <- finance_point %>%
  mutate(NimU = ifelse(is.na(NimU), -25, NimU))
finance_point %>%
  ggplot(aes(RmU,Em))+
  geom_point() +
  theme(axis.text.x=element_text(angle=60,hjust = 1)) +
